...
export BRANCH=$4 # branch or test name (configurable / non-secure / MCX)
...
RESULTS_DIR=$CUR_DIR/results/$BRANCH/$BMK-$SIZE/$CONFIG-$META_SIZE

$CUR_DIR/build/X86/gem5.opt -d $RESULTS_DIR $CUR_DIR/configs/example/gem5_library/$CONFIG --benchmark $BMK --size $SIZE --metadata_cache_size $META_SIZE --cache_mac --l3_size 256KiB --mcx_policy never
